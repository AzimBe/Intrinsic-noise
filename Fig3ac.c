...
#define V        187.4389 //23.43 //187.44 //93.72  //187.44     // cell volume
#define xc       223 //28 //223 //111 //223        // number of moelcules at stable point c
#define xa       40 //5 //40 //20 //40  // number of moelcules at stable point a
...
void init( int x[], double c[], double T){
  // population of chemical species
  x[0] = 0;       // mRNA
  x[1] = xc;      // protein monomer
  //x[2] = 0; 
  pt = x[1];//+2*x[2]  ;

  // parameters
  c[0] = log(2.0)/T;            //  gamma_p
  c[1] = 20./T;                 //   k_p
...
int main(void){  
	...
	FILE * fp;
        fp = fopen ("ac1t101.m", "w+");
	fprintf(fp, "function xi = ac1t101\n");
	fprintf(fp, "xi = [\n");
	for(i=0; i < 5; i++){
